...
## 代码块
:::preview
``` cpp title="demo" 	:collapsed-lines = 17
#include <iostream>
...
| highlighter options | 折叠   | :collapsed-lines<br>:no-collapsed-lines<br>:collapsed-lines=17 //从17行开始换行 | collapsedLines        |
|                     | 标题   | \`\`\`  cpp title="demo"                                                  |                       |
...
15
```
当代码块行数大于等于15行时会出现折叠按钮
2. 能否配置启用折叠的默认行数？
似乎不行
3. :collapsed-lines的使用
```
1
2
3
...
如果没有配置 `collapsedLines: true`，默认是不会折叠的。
高亮器选项：\`\`\` 后，代码块语言、标题 配置行
:::preview
```txt :collapsed-lines 启用折叠
。。。
```
:::
4. 如果想要折叠代码块中间位置的内容，或者 有多个区域需要折叠
按照我的想法
```
开头
...
结尾
```
从collapsed ~ no-collapsed 之间的内容应该被折叠，显示为 。。。
当然实际是不行，只能分成多个代码块
